...
Derive `LocationClass` per unit from path heuristics: frontend source roots, built-bundle output dirs, and `public/` → `ClientReachable`; server env files, edge-function dirs, server-only roots → `ServerOnly`; otherwise `Unknown`. This is heuristic and feeds severity, not gospel — a `secret`-type key is Critical regardless of location; the class only sharpens the *anon/publishable* judgement and the messaging.
...
Uses only the public key (publishable/anon) already discovered in the bundle, plus the project URL. Enumerates tables from the PostgREST OpenAPI description, then issues a **read-only** `select … limit 1` per table as the anon role. Rows returned ⇒ RLS is off or non-filtering ⇒ Critical, with the reproduction attached (endpoint + observed row count; never the row contents). This mirrors exactly what any visitor's browser can already do, uses no elevated credential, and is read-only. It is permitted in the default local scan **only when the user has enabled network probing** (opt-in flag), and only against a project whose key the tool found in the user's own repo.
...
- **Table enumeration:** obtain the list of API-exposed tables from the project's PostgREST OpenAPI description using the public key.
- **Tier 0 (read probe):** per table, read-only anon `select limit 1`; classify each as `Protected` (no rows / filtered) or `Exposed` (rows returned). Attach reproduction metadata (endpoint, observed row count), never row contents. Emit Critical `Rls` findings for `Exposed`.
