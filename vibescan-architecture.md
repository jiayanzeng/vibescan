...
- **Primary channel:** the release owner's personal-scope `@jiayanzeng/vibescan` npm wrapper package ships (never downloads at install time) the correct prebuilt binary per platform so `npx @jiayanzeng/vibescan` works for the JS-native audience. The unscoped `vibescan` name is third-party-owned, and no separately controlled `@vibescan` organization scope is available; neither is a publication target. Secondary: `cargo install`, Homebrew.
