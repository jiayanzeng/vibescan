...
prebuilds painless — the tool is architected for `npx vibescan` but cannot yet be
installed that way.
...
- The primary channel: an npm wrapper package that ships/downloads the correct
  prebuilt per platform so `npx vibescan` works. Secondary: `cargo install`,
  Homebrew.
